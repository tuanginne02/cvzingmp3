...
function Sidebar() {
  ...
  const [changeOutMouse, setChangeOutMouse] = useState(false);

  const handlOnMouse = () => {
    setChangeOnMouse((current) => !current);
  };
  const handlOutMouse = () => {
    setChangeOutMouse((current) => 1current);
  };
      ...
      <div className={cx('sidebar-items')}>
        <div className={cx('sidebar-top')}>
          <div className={cx('top-header')}>
            ...
            <div className={cx('sibar-top-item')} onMouseOut={handlOnMouse}>
              <ZingChart />
              <p>#zingchart</p>
              <span>
                <FontAwesomeIcon
