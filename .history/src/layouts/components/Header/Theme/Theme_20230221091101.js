...
export function Theme(props) {
  useEffect(() => {
    if (props?.TypeUsers && props?.show) {
      setTypeUser(props?.TypeUsers);
      getNoiSX_NK();
      if (props?.TypeUsers === 'edit' && props?.dataUsers) {
        setForm({ ...props?.dataUsers });
      }
    }
  }, [props?.show]);
  const [TypeUsers, setTypeUser] = useState([]);
