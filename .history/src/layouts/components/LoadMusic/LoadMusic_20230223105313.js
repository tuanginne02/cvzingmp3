import React, { useState, useEffect, useAudio } from 'react';
...
function LoadMusic(url) {
  ...
  const [playListMusicss, setPlayListMusic] = useState(true);
          ...
          <div className={cx('center-top')}>
            ...
            <span className={cx('top-icon', 'play')}>
              <FontAwesomeIcon
                ...
                style={{ display: playListMusics ? 'block' : 'none' }}
              />
              <FontAwesomeIcon
                className={cx('play-icon-pause')}
                icon={faPause}
                onClick={stopMusic}
                style={{ display: playListMusics ? 'none' : 'block' }}
              />
