...
function LoadMusic() {
  ...
  const [playLine, setPlayLine] = useState(true);

  const playMusic = () => setPlayListMusic(false);
  const stopMusic = () => setPlayListMusic(true);
  const playLine = () => setPlayListMusic(false);
  const stopLine = () => setPlayListMusic(true);



  const [autoCouter, setAutoCouter] = useState(0);
  let timer;
  // useEffect(() => {
  //   clearInterval(timer)
  //   timer = setInterval(() => {
        ...
        <div className={cx('content-center')}>
          ...
          <div className={cx('center-bottom')}>
            ...
            <div className={cx('center-bottom-line')}>
              <div className={cx('line-run')} style={{ animation: playLine ? 'none' : '' }}>
