...
function MVVN() {
  ...
  const toggleList = () => setFillterList((current) => (!current))
        ...
        <div className={cx('vn-header')}>
          <div className={cx('header-filler')}>
            <div className={cx('btn-all')}>
              ...
              <p>Tất Cả</p>
              <div className={cx('icon-search')} onClick={toggleList}>
                <FontAwesomeIcon className={cx('list-search')} icon={faChevronDown} style={{
        display: closeModal ? 'none' : '',
      }}/>
                <FontAwesomeIcon className={cx('list-search')} icon={faChevronUp} style={{
        display: closeModal ? 'none' : '',
      }}/>
              </div>
            </div>
